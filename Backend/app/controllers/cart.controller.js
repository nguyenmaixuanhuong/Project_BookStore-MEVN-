...
exports.createCart = async (req, res, next) => {
    ...
    try {
        ...
        if (cart) {
            ...
            if (itemIndex > -1) {
                return res.send({inCart:true})
            }
            else {
                cart.products.push({ productId, quantity, totalprice, image: image.imageName });
            }
            ...
            const newCart = await Cart.create({
                user: iduser,
                products: [{ productId, quantity, totalprice, image: image.imageName }]
            });
